...
export default function AdminFileRequestsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get(`/api/admin/file-requests?${params}`);
      setRequests(data.fileRequests);
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">File requests</h1><p className="text-muted-foreground">Upload request links</p></div>
      {error && <div className="rounded-md bg-destructive/15 p-3 text-sm text-destructive">{error}</div>}
      <Card>
        <CardContent className="pt-6">
          {loadingData ? <p className="py-8 text-center text-muted-foreground">Loading...</p> :
          requests.length === 0 ? <p className="py-8 text-center text-muted-foreground">No file requests</p> : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b">
                    ...
                    <th className="text-left py-3 px-2 font-medium">Max files</th>
                  </tr></thead>
                  <tbody>
                    {requests.map((r) => (
                      <tr key={r.id} className="border-b hover:bg-muted/50">
                        <td className="py-3 px-2 font-mono text-xs">{r.token.slice(0, 12)}...</td>
                        <td className="py-3 px-2">{r.userEmail ?? '—'}</td>
                        <td className="py-3 px-2">{r.expired ? <Badge variant="destructive">Expired</Badge> : <Badge variant="secondary">Active</Badge>}</td>
                        <td className="py-3 px-2">{new Date(r.expiresAt).toLocaleDateString()}</td>
                        <td className="py-3 px-2">{r.maxFiles ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
